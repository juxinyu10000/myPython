...
def data_prepration(x):
    ...
    x_features_train,x_features_test,x_labels_train,x_labels_test = train_test_split(x_features,x_labels,test_size=del_precent)  #****抽样比例可以自己设置
...
#设置过滤属性列
drop_list = ['member_id','term','sub_grade','emp_title','issue_d','title','zip_code','addr_state','earliest_cr_line','last_pymnt_d','last_credit_pull_d','url','pymnt_plan','initial_list_status']
...
loans["loan_status"] = coding(loans["loan_status"], {"Current":0,"Issued":0,"Fully Paid":0,"In Grace Period":1,"Late (31-120 days)":1,"Late (16-30 days)":1,"Charged Off":1,"Does not meet the credit policy. Status:Charged Off":1,"Does not meet the credit policy. Status:Fully Paid":0,"Default":0})#打印状态信息
...
drop_col = ['funded_amnt', 'funded_amnt_inv', 'installment', 'out_prncp_inv', 'total_pymnt_inv', 'total_rec_prncp','total_rec_int', 'verification_status_Not Verified', 'verification_status_Source Verified','collection_recovery_fee', 'verification_status_Verified','application_type_JOINT']
...
pd.value_counts(data_test_y["loan_status"])
